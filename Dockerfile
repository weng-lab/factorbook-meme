...
RUN apt-get update && apt-get install -y \
    ...
    cd meme-5.0.4 && ./configure --enable-build-libxml2 --enable-build-libxslt --prefix=/bin/meme_install && \
    make && make install && cp /bin/meme_install/bin/* /bin && cp /bin/meme_install/libexec/meme-5.0.4/* /bin && \
