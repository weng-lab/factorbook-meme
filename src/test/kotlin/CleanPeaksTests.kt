...
class CleanPeaksTests {
    ...
    @Test fun `Test cleanPeaks without methyl bed`() {
        cleanPeaks(PEAKS, TEST_CHR_FILTER, null, testOutputDir.resolve(CLEANED_PEAKS), setOf())
    ...
    @Test fun `Test cleanPeaks with methyl bed`() {
        ...
        cleanPeaks(M_PEAKS, null, methylData, testOutputDir.resolve(M_CLEANED_PEAKS), setOf())
    ...
    @Test fun `Test cleanPeaks with multiple methyl beds`() {
        ...
        cleanPeaks(M2_PEAKS, null, methylData, testOutputDir.resolve(M2_CLEANED_PEAKS), setOf())
